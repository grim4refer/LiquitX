...
public class ClanChatManager {
	...
	public static void init() {
		try {
			if (new File(FILE_DIRECTORY).listFiles() == null) {
				System.out.println("Missing LiquidX.json in ClanChat directory. Exiting.");
				System.exit(0);
			}
			for (File file : (new File(FILE_DIRECTORY)).listFiles()) {
				...
				if (file.getName().equalsIgnoreCase("LiquidX.json")) {
					file.setReadable(true);
					file.setReadOnly();
					System.out.println("Set \"LiquidX.json\" to Read Only. (Anti-corruption)");
				}
	...
	public static void writeFile(ClanChat clan) {
		try {
			if (clan.getName().equalsIgnoreCase("LiquidX")) {
				System.out.println("Skipped writing LiquidX.json file because it's read only.");
				return;
			}
